...
export function TodoBlock({
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      if (e.key === 'Enter' && !e.shiftKey) {
        ...
        // If todo is empty, convert to body block
        if (content.trim() === '') {
          if (onConvertToBody) {
            onConvertToBody();
          }
        ...
        // If todo has content, create new todo block
  ...
  const handleBlockClick = useCallback((e: React.MouseEvent) => {
    // If click was on checkbox, let it handle itself
    if ((e.target as HTMLElement).classList.contains('todo-checkbox')) return;
  ...
  return (
    <div
      className={`block todo-block ${isFocused ? 'focused' : ''} ${checked ? 'checked' : ''}`}
    ...
    >
      ...
      <div
        ...
        className="todo-content editable-block-content"
      />
      {isFocused && (
